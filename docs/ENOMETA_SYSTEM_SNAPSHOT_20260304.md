...
- **음악**: Raw Synthesis 전자음악 (Pure Python v11 엔진, 9종 무드 선택제 + 패턴 엔진 + 텍스처 모듈, GPU 불필요)
...
[STEP 6] 영상 제작 파이프라인
         → script.txt → TTS 실측 타이밍 → BGM(연속, TTS 총 길이+엔드카드) → 믹싱
         → Python 비주얼 렌더링(hybrid) → Remotion 합성
```
...
- v11: enometa 기반 + 9종 무드 선택제 (ambient/ikeda/experimental/minimal/chill/glitch/raw/intense/techno) + `render_mode: "hybrid"` + `frames_dir`/`total_frames` 자동 계산
...
### 3-5. 오디오 믹싱 / FFT
- 오디오 믹싱: narration 90% + BGM 100%, **사이드체인 없음**
- output_duration = max(narration, bgm) — BGM이 더 길면 엔드카드까지 이어짐
- EBU R128 loudnorm: `-14 LUFS / TP=-1.5dB / LRA=11` (클리핑 방지, TTS/BGM 균형 자동 처리)
- **D-4 동적 믹싱** (선택): `--dynamic-mix script_data.json` → SI 기반 BGM 볼륨 미세 조절 (최대 ±15%)
  - `BGM 볼륨 = bgm_base × (1.0 - 0.15 × si)` — si=0→100%, si=1→85%
  - 기본 OFF (기존 고정 볼륨 유지), narration_volume 0.90은 절대 고정
...
- **LogoEndcard v3**: 파티클 수렴 + 태그라인 전면 강화
  - v2: 검정 화면 축소, 수렴 가속, 파티클 다이나미즘, TitleSection fade-out 연동
  - **v3 (EP007)**: 태그라인 fontSize 30→48, fontWeight 400→700, 글자별 스태거 애니메이션
  - SVG 밑줄 드로잉 + 파티클 180개 수렴/호흡 효과
  - letterSpacing 0.3em, opacity max 0.85
...
| | LightSource | 빛, 희망, 초월 |
| **타이포그래피** | TextReveal | 텍스트 애니메이션 (4모드: typewriter/wave/glitch/scatter) |
| **데이터 시각화** | DataBar | 바/링 차트 |
| | Lissajous | Lissajous 곡선 — 수학적 기하학 패턴 (EP007 신규) |
| **Max Cooper** | GridMorph | 격자 왜곡 (3모드) |
| **오디오 파형** | WaveformVisualizer | 파형 시각화 (4모드) |
| **8bit** | PixelGrid | 8bit 격자 (fill/outline/life/rain 4모드) — zIndex: 5 |
| | PixelWaveform | 8bit 파형 (bars/steps/cascade 3모드) |
| **기하학** | ShapeMotion | emotion별 기하학적 도형 (5패턴) |
| **오버레이** | PostProcess | 비네트, 스캔라인, onset 플래시 — zIndex: 10 |

### z-order 시스템 (EP007 신규)
| 레이어 | zIndex | 비고 |
|--------|--------|------|
| 일반 vocab | auto | 기본 DOM 순서 |
| PixelGrid | 5 | 다른 vocab 위에 코드 비주얼 부각 |
| PostProcess | 10 | 최상위 포스트프로세싱 |

### VOCAB_MAP (38+ vocab 키)
```
기존 30개 + 8bit 7개 + lissajous 1개:
...
## 5. 음악 엔진 v11 + v15 무드 시스템

### 장르/무드 시스템 (v15: 9종 선택제)

| 장르 | BPM | 핵심 | 비고 |
|------|-----|------|------|
| **enometa** | **135 (±20%)** | **10레이어 + 패턴 엔진(DRUM_PATTERNS/SAW_PATTERNS) + 호흡 + SI 80~105% 변조** | **기본 엔진** |

**음악 무드 9종**: ambient / ikeda / experimental / minimal / chill / glitch / raw / intense / techno
- 무드별 레이어 ON/OFF 조합으로 에너지 표현 (볼륨 변조 ±15% 이내)
- 사용자가 에피소드마다 무드 선택

**비주얼 무드 4종**: ikeda / cooper / abstract / data
...
### v6.1 Song Arc 시스템 (기승전결) — v11 D-3b: 7종
음악 1곡 풀버전에 매크로 에너지 엔벨로프 적용 (기존 섹션 시스템 위의 상위 레이어)

| 아크 | 구간 | 설명 |
|------|------|------|
| **narrative** | intro(0-15%)→buildup(15-55%)→climax(55-80%)→outro(80-100%) | 기승전결 (기본) |
| crescendo | grow(0-85%)→release(85-100%) | 점진 성장 |
| flat | constant(0-100%) | 아크 없음 (하위호환) |
| adaptive | si 곡선 기반 동적 경계 | 대본과 정렬 |
| **wave** | rise1(0-25%)→dip(25-45%)→rise2(45-75%)→fade(75-100%) | 파동 (이중 나선 구조C 적합) |
| **shockwave** | spike(0-15%)→decay(15-55%)→aftershock(55-75%)→settle(75-100%) | 충격파 (역순 각성 구조B 적합) |
| **staircase** | step1(0-25%)→step2(25-50%)→step3(50-75%)→step4(75-100%) | 계단 (점진형 변형) |

- CLI: `--arc narrative|crescendo|flat|adaptive|wave|shockwave|staircase` (기본: narrative)
...
│   │   ├── SubtitleSection.tsx         # EP005 원본 기반 + splitSegmentToSentences() 문장 분할
│   │   │                               #   fadeIn/Out 0.2s, smartLineBreak(18), 35자 분할
│   │   ├── ShapeMotion.tsx             # emotion별 기하학적 도형 (5패턴)
│   │   │                               #   tension=회전사각/climax=동심원/awakening=스캔라인/intro=부유점/buildup=삼각펄스
│   │   └── vocab/                      # 23개 비주얼 컴포넌트
│   │       ├── (기존 20개)
...
v7-P8  [████████░░] 연속 악기 si 게이트: 연속 함수(min 0.45, 1.0s 스무딩) — 계단 함수/급감쇠 금지
...
v16    [█████████░] TTS 실측 타이밍 (마디 snap 제거) + 9종 무드 선택제 + techno 무드
v11-D  [█████████░] 창작 자유도 감사 D섹션 — 글쓰기+오디오 확장
       — 글쓰기 3종 구조(A:3단계변환 / B:역순각성 / C:이중나선)
       — 도메인 5+5 (코어5 + 확장5: 물리학/생물학/수학/경제학/예술)
       — SI 커브 7종 (표준/크레셴도/역전/파동/충격파/계단/미니멀)
       — SONG_ARC_PRESETS 7종 (+wave/shockwave/staircase)
       — audio_mixer --dynamic-mix (SI 기반 BGM 볼륨 ±15%)
...
[3] docs/FEEDBACK_LOG.md  →  전 에피소드 피드백 통합 로그 (사람이 읽는 뷰)
[4] ENOMETA_ClaudeCode_Brief + MEMORY.md + SYSTEM_SNAPSHOT  →  문서 최신화
```
