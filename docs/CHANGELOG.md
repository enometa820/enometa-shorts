...
### Added
- `CLAUDE.md`: **미학 원칙** 섹션 신설 — 백남준 DNA (원초적+불완전, 너드+키치, 고감도 반응, 죽은 코드 금지)
- `PostProcess.tsx`: CRT 수평 티어링 (onset 시 5개 수평 스트립), 노이즈 그레인 (SVG feTurbulence), 인터레이스 플리커 (3프레임 주기)
- `visual-architecture.md`: 미학 원칙 섹션 + PostProcess v22 수치 비교표

### Changed
- `PostProcess.tsx`: 스캔라인 4배 강화 (2.4%→9.6%), 색수차 5배 확대 (3px→12-20px), onset 플래시 강화
- `SymbolMotion.tsx`: **전면 교체** — SVG 추상 도형 → ASCII 캐릭터 그리드 (28col×14row, POS별 패턴, onset 40% 글리치)
- `AsciiArt.tsx`: 전 모드 크기 ~2배 (block cellSize 28→42, shape fontSize 24→36, matrix 26→34), 글리치 확률 60%, 호흡 ±8%+rms, glow 이중화

---

## 2026-03-10 — 글쓰기 스킬 v13: 감성 레거시 제거 + 컴퓨팅 아이덴티티

### Changed
- `enometa-writing.md`: v12→v13 전면 개정
  - §1 아이덴티티: "팩트로 위로하는" → "팩트로 재구성하는" (Reconfigure Through Facts)
  - §2 톤: "담백한 격조" → "차가운 정밀" — 감정 배제, 데이터가 말하게, 건조하되 리듬
  - §2 인칭: "우리" 강제 → 3인칭 관찰 기본 ("뇌는", "시스템은")
  - §3 도메인: 코어5+확장5 → 컴퓨팅 렌즈(고정) + 소재 도메인 7종
  - §4 구조: 3단계변환/역순각성/이중나선 → 데이터 파이프라인/에러 로그/병렬 프로세스
  - §8 허용/금지: 감정 호소 금지 추가
  - §9 펀치라인: 감정 기반 → 시스템에러/디버그/리팩토링/패턴인식/구조분해형
  - §12 채점: "감정 곡선" → "인텐시티 곡선", "공유하고 싶은가" → "인식 전환이 발생하는가"
  - §14 체크리스트: "인칭 일관성" → "인칭 적절성"
- `CLAUDE.md`: 글쓰기 규칙에 v13 정보 반영

### Unchanged (보존)
- §0 (1문장=1마디), §5 (SI 커브 7종), §6 (비주얼 vocab), §7 (음악 연동), §10 (분량), §11 (제목), §13 (워크플로우 STEP 0~4), publish.md 템플릿
