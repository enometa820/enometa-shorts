...
| vocab | 설명 |
|-------|------|
| `symbol_morph` | 품사에 따라 추상 도형 변환 (명사→육각형, 동사→화살표, 형용사→물결) |
| `ascii_block` | 텍스트를 5×7 비트맵 블록으로 렌더링 |
| `ascii_shape` | 품사별 ASCII 패턴 |
| `ascii_matrix` | 터미널 데이터 스트림 (Matrix 스타일) |
...
#### 2. 스캔라인 (Scanline)
4px 주기로 미세한 수평선이 항상 그려진다. CRT 모니터 감성.
opacity 약 3.6%로 매우 은은하다.

```
효과: 4px 주기 수평선 → CRT 텍스처
opacity: 고정 (음악 반응 없음)
```
...
#### 5. 색수차 (Chromatic Aberration)
화면 좌우 가장자리에서 빨강(왼쪽) / 파랑(오른쪽) 색 분리가 일어난다.
고주파 사운드(하이햇, 심벌)에 반응한다.

```
강도 = 0.04 + audio.high × 0.1
고주파 없음 → 4% 색 분리
강한 고주파 → 14% 색 분리
분포: 상하 30~70% 구간에 집중
```
